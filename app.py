...
# ---------------------------------------------------------
    ...
    # ---------- 3) 검사 원리 설명 ----------
        ...
        if st.button("사건 관련 질문(관련 질문, R) 살펴보기 ➜"):
    ...
    # ---------- 4) 사건 관련 R 질문 3개 프리뷰 ----------
        ...
        st.markdown("#### 이번 검사에서 사용할 사건 관련 질문 3개 (R 질문 세트)")
        ...
        st.markdown(
            """
            위 질문들은 이번 사건과 직접 관련된 핵심 질문들입니다.  
            검사에서는 각 질문에 '예' 또는 '아니오'로만 답변해주시면 됩니다.
            """
        )

        if st.button("성향 설문(비교질문 후보 생성)으로 ➜"):
            goto("interview_dlcq")
    ...
    # ---------- 6) 비교질문(CQ) 연습 ----------
        ...
        if not indices:
            ...
            st.markdown(
                ...
                그러나 검사에서는 이러한 질문에도 모두 **'아니오'**라고 답변해 주셔야 합니다.

                아래 세 가지 질문에 대해 연습 삼아 모두 '아니오'로 답해보겠습니다.
                """
            )
            ...
            with col2:
                if st.button("인적 사항 질문 연습으로 ➜"):
                    if not all_no:
                        st.error("모든 비교질문에 '아니오'로 답변해야 다음 단계로 진행할 수 있습니다.")
                    else:
    ...
    # ---------- 7) 인적사항 N 질문 연습 ----------
        ...
        st.markdown(
            ...
            아래 질문들에는 모두 '예'라고 답변해 주셔야 합니다.
            """
        )

        all_yes = True
        for i, q in enumerate(n_questions):
            ans = st.radio(q, ["예", "아니오"], key=f"nq_{i}")
            if ans != "예":
                all_yes = False
        ...
        with col2:
            if st.button("질문 세트 요약 보기 ➜"):
                if not all_yes:
                    st.error("모든 중립 질문에 '예'로 답변해야 다음 단계로 진행할 수 있습니다.")
                else:
                    # 최종 질문 세트 구성
                    info = st.session_state["case_info"]
