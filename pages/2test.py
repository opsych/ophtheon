...
def build_full_script(seq):
    ...
    script_parts.append(
        ...
        "화면 중앙의 십자를 조용히 응시해 주세요. "
        "눈을 장시간 감거나 자주 깜빡이지 않도록 노력해 주시고, 몸을 최대한 움직이지 말아 주세요. "
        "연습했던 열한 개의 질문이 순서대로 제시됩니다. "
    ...
    for i, item in enumerate(seq, start=1):
        ...
        script_parts.append(
            ...
            f"잠시 후 다음 질문을 읽어 드리겠습니다. 천천히 내용을 들으신 뒤, 삼 초 후에 예 또는 아니오로 대답해 주세요. {q_text} . 삼, 이이, 일."
        )
...
# ---------- (1) 텍스트 파일 업로드 ----------
if step == "upload":
    st.title("검사 시행 — 질문 불러오기")

    st.markdown(
        """
검사 전 단계에서 생성한  
**질문 텍스트(.txt)** 파일을 업로드해 주세요.
        """
    )

    uploaded = st.file_uploader("질문 텍스트 파일(.txt) 업로드", type=["txt"])
    ...
    if uploaded is not None:
        ...
        st.markdown("질문이 로드되었습니다. 이제 검사 질문을 생성합니다.")
...
화면에는 **회색 배경과 십자(+) 표시만** 나타납니다.  
오디오를 재생한 후, 중앙에 나타나는 십자 표시를 응시해야 합니다.  
총 11개의 질문이 자동으로 재생됩니다.  

질문을 들을 때마다,  
**또렷한 목소리로 '예' 또는 '아니오'** 라고 대답해주세요.

Ophtheon은 자동 음성으로 질문을 안내합니다.  
일부 억양이나 발음이 부자연스럽게 들릴 수 있지만 정상적인 현상이며,  
검사에 영향을 주지 않으니 편안하게 진행해주세요.
            """
        )
