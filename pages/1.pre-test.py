...
# ⚠ set_page_config는 app.py에서만 호출하는 게 안전해서 여기서는 뺐어.
# (여기서 또 호출하면 "한 번만 호출해야 한다"는 경고가 뜰 수 있음)
...
if "final_shuffle_order" not in st.session_state:
    # 나중에 최종 연습에서 쓸 수도 있으니 기본값만 만들어 둠
    st.session_state["final_shuffle_order"] = None
