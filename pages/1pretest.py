...
if step == "interview_info":
    ...
    if offense_category == "실험참여":
        offense_type = st.selectbox(
            "사건의 세부유형을 선택해 주세요.",
            ["빨간 버튼을 클릭", "인형을 폭행"],
        )
    elif offense_category == "성범죄":
        offense_type = st.selectbox(
            "사건의 세부유형을 선택해 주세요.",
            ["성희롱", "강제추행", "강간", "불법촬영"],
        )
    ...
    if offense_type == "기타":
        offense_free = st.text_input(
            "어떤 행위에 관한 사건인가요?",
            placeholder="마약류 투약, 마약류 밀매, 미성년자 유인, 성매매 등",
        )
...
다음 화면에 세 문항이 제시되면,  
실제 검사와 같은 방식으로 예/아니오 응답을 연습해 보겠습니다.
        """
    )
...
방금 설문에서는 해당 문항들에 **'예'라고 답하셨지만**,  
앞으로의 **질문에서는 이 세 문항에 모두 '아니오'라고 답변**하셔야 합니다.

다음 화면에서 선택된 세 문항을 보여드리겠습니다.  
각 문항을 읽어보시고,  
**'예'라고 했던 내용을 '아니오'라고 답해보는 연습**을 하겠습니다.  

이 질문은 사건 관련 질문만큼 중요하기에,  
반드시 집중해서 '아니오'로 답변해주세요.  

다음 화면에 제시되는 **세 문항에 대해서는**,  
이후 **최종 연습과, 본 검사에서도 '아니오'라고** 답변해주시면 됩니다.
            """
        )
...
다음 화면에서 세 문항이 제시되면, 사실대로 응답해 주시면 됩니다.
        """
    )
...
이제 실제 검사에 앞서,  
지금까지 연습한 모든 질문,  
즉, 검사에서 진행될 **11가지 질문을 한 번에 연습**해 보겠습니다.

다음 화면에서는 질문이 랜덤한 순서로 제시됩니다.

연습했던 것과 같이,  
사실임에도 '아니오'라고 답변해야 하는 질문이 있으니,  
주의해서 답변해주세요.
        """
    )
...
텍스트 파일로 저장한 뒤 검사 시행 단계에서 제출해 주세요.
            """
        )
